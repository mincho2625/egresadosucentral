<!DOCTYPE html>

<html lang="es">


<head>
	<title>Informacion Personal</title>
	<meta charset="utf-8"/>
	
	<link rel="stylesheet" href="estilo.css">

</head>
<body>
<div id="Tabla" align="center">

<s:form method="post" action=".jsp" name="f1">
 
        <table rules="all" >
           <caption>DATOS DE UBICACION</caption>
            <tr>
              <td class="tddos">Tipo Contacto<br>
              </td>
              <td class="tddos">Descripcion<br>
              </td>
              <td class="tddos">Fecha<br>
              </td>
              <td class="tddos">Accion<br>
              </td>
            </tr>
            <tr>
              <td class="tdtres">ESTO<br>
              </td>
              <td class="tdtres">Que esta qui<br>
              </td>
              <td class="tdtres">es un gran ejemplo<br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
        </table>
        <br>
        <div style="text-align: right;"><input name="ubicacion" value="Agregar Otra Ubicacion" type="button"><br>
        </div>
        <br>
        <br>        
        <table rules="all">
          <caption>REDES SOCIALES</caption>
            <tr>
              <td class="tddos">Red Social<br>
              </td>
              <td class="tddos">Descripcion<br>
              </td>
              <td class="tddos">Fecha<br>
              </td>
              <td class="tddos">Accion<br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
            <tr>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><br>
              </td>
              <td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
              </td>
            </tr>
        </table>
        <br>
        <div style="text-align: right;"><input name="redSocial" value="Agregar Otra Red Social" type="button"><br>
        </div>
<br><br>

<table rules="all">
<caption>DATOS DE RESIDENCIA</caption>
<tr>
<td class="tddos">Tipo Vivienda<br>
</td>
<td class="tddos">Tendencia<br>
</td>
<td class="tddos">Fecha<br>
</td>
<td class="tddos">Accion<br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img style="width: 20px; height: 19px;" title="Editar" alt="Editar" src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
</td>
</tr>
</table>
<br>
<div style="text-align: right;"><input name="residencia" value="Agregar Otra Residencia" type="button"><br>
</div>
</s:form>

</div>	
</body>
</html>