<!DOCTYPE html>

<html lang="es">


<head>
	<title>Educacion Formal</title>
	<meta charset="utf-8"/>
	
	<link rel="stylesheet" href="estilo.css">

</head>
<body>
<div id="Tabla" align="center">
<form method="post" action=".jsp" name="f1">
<table rules="all">
<caption>DATOS ACADEMICOS UCENTRAL</caption>
<tr>
<td class="tddos">Titulo<br>
</td>
<td class="tddos">Nivel<br>
</td>
<td class="tddos">Ingreso<br>
</td>
<td class="tddos">Finalizacion<br>
</td>
<td class="tddos">Accion<br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"><br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
</table>
<br>
<div style="text-align: right;"><input name="consulta"
value="Consultar" type="button"><br>
</div>
<br>
<br>
<table rules="all">
<caption>INFORMACION ACADEMICA OTRAS INSTITUCIONES</caption>
<tr>
<td class="tddos">Titulo<br>
</td>
<td class="tddos">Institucion<br>
</td>
<td class="tddos">Ingreso<br>
</td>
<td class="tddos">Finalizacion<br>
</td>
<td class="tddos">Accion<br>
</td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td ><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
<tr>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><br>
</td>
<td class="tdtres"><img
style="width: 20px; height: 19px;" title="Editar" alt="Editar"
src="../plantillaHTML/imagenes/Icono_de_editar.png" align="top"> </td>
</tr>
</table>
<br>
<div style="text-align: right;"><input name="agregarInstitucion"
value="Agregar Otro Registro Academico" type="button"><br>
</div>
</form>
</div>	
</body>
</html>